...
/**
 * FLAT BULANAN
 * Pastikan kolom ini ada di tabel users:
 * - daya_va
 * - tagihan_bulanan
 */
$dayaVA = (int)($u['daya_va'] ?? 0);
...
<body>
  <div class="receipt-page">
    <div class="receipt" id="receipt">
      <h1>BUKTI PEMBAYARAN TAGIHAN LISTRIK</h1>
      ...
      <div class="grid">
        ...
        <div class="box">
          ...
          <?php $isLunas = (strtoupper($status) === 'LUNAS'); ?>
          <div class="status-pill <?= $isLunas ? 'lunas' : 'belum' ?>">
            <?= htmlspecialchars($status) ?>
          </div>
        </div>

        <!-- BLOK TAGIHAN: FLAT BULANAN -->
        <div class="box full">
          <div class="split">
            <span class="muted">Daya Listrik</span>
            <b><?= $dayaVA ? number_format($dayaVA,0,',','.') . " VA" : "-" ?></b>
          </div>

          <div class="split">
            <span class="muted">Jenis Tarif</span>
            <b>Flat Bulanan</b>
          </div>

          <div class="total">
            <span>Total Tagihan</span>
            <span>Rp <?= number_format($total,0,',','.') ?>/bulan</span>
          </div>
        </div>
      </div>
